...
def read_table(file_name=None):
    ...
    path to file needs to be added in the environment variable
        GCP_SERVICE_ACCOUNT_DATALAB_JUSTICIA
    Args:
    ...
    """
    if not os.getenv('GCP_SERVICE_ACCOUNT_DATALAB'):
        raise ValueError('Set the environment variable GCP_SERVICE_ACCOUNT_DATALAB_JUSTICIA'
                         ' to the json file containing the gcp credentials')
...
def get_gcp_bucket():
    credentials_file = os.environ['GCP_SERVICE_ACCOUNT_DATALAB']
